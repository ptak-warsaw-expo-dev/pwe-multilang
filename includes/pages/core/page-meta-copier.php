...
final class PWE_Multilang_Page_Meta_Copier
{
    ...
    private static function should_skip_meta_key(string $meta_key): bool
    {
        ...
        $excluded_meta_keys = [
            ...
            '_pwe_multilang_creation_state',

            // Copied explicitly by copy_wp_rocket_page_meta().
    ...
    private static function remove_uncode_header_meta_from_target(int $target_id): void
    {
        ...
        foreach (array_keys($target_meta) as $meta_key) {
            ...
            if (self::is_uncode_header_meta_key($meta_key)) {
                delete_post_meta($target_id, $meta_key);
            }
        }
    }
    ...
    {
        PWE_Multilang_Page_Meta_Adapter::set_uncode_header_none($target_id);
    }

    private static function is_uncode_header_meta_key(string $meta_key): bool
    {
        $lower_meta_key = strtolower($meta_key);

        if ($lower_meta_key === PWE_Multilang_Page_Meta_Adapter::UNCODE_HEADER_META) {
            return true;
        }

        if ($lower_meta_key === PWE_Multilang_Page_Meta_Adapter::UNCODE_HEADER_BLOCK_META) {
            return true;
        }
        ...
        foreach (['header', 'headers', 'page_header', 'page-header', 'header_block', 'header-block'] as $fragment) {
            if (strpos($lower_meta_key, $fragment) !== false) {
                return true;
            }
        }
